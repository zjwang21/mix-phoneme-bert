...
class MixPhonemeTokenBlockDataset(FairseqDataset):
    ...
    def __init__(
        ...
        super().__init__()
        self.dataset_sp = dataset_sp
        self.dataset_p = dataset_p
        self.dataset_wwm = dataset_wwm
        self.pad = pad
        self.eos = eos
        self.bos = bos
        self.include_targets = include_targets

        assert len(dataset_sp) > 0
        assert len(dataset_p) > 0
        assert len(dataset_sp) == len(dataset_p)
        _sizes, block_to_dataset_index, slice_indices = self._build_slice_indices(
            sizes, break_mode, document_sep_len, block_size
        )
        if use_plasma_view:
            plasma_id = (block_size, document_sep_len, str(break_mode), len(dataset_sp))
    ...
    def __getitem__(self, index):
        ...
        buffer_sp = torch.cat(
            [self.dataset_sp[idx] for idx in range(start_ds_idx, end_ds_idx + 1)]
        )
        buffer_p = torch.cat(
            [self.dataset_p[idx] for idx in range(start_ds_idx, end_ds_idx + 1)]
        )
        buffer_wwm = torch.cat(
            [self.dataset_wwm[idx] for idx in range(start_ds_idx, end_ds_idx + 1)]
        )
        slice_s, slice_e = self.slice_indices[index]
        length = slice_e - slice_s
        s, e = start_offset, start_offset + length
        item_sp = buffer_sp[s:e]
        item_p = buffer_p[s:e]
        item_wwm = buffer_wwm[s:e]
        item_sp = torch.cat([item_sp.new([self.bos]), item_sp, item_sp.new([self.eos])])
        item_p = torch.cat([item_p.new([self.bos]), item_p, item_p.new([self.eos])])
        item_wwm = torch.cat([item_wwm.new([self.bos]), item_wwm, item_wwm.new([self.eos])])
        return {"sup_phoneme": item_sp, "phoneme": item_p, "wwm": item_wwm}
